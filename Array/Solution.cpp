...
public:
    long long maxAlternatingSum(vector<int>& input, vector<vector<int>>& swaps) const {
        ...
        vector<ConnectedComponents> parentToConnectedComponents
...
private:
    vector<ConnectedComponents> createMapParentToConnectedComponents(span<const int>input, UnionFind& unionFind) const {
        vector<ConnectedComponents> parentToConnectedComponents(input.size());
    ...
    long long calculateMaxAlternatingSum(vector<ConnectedComponents>& parentToConnectedComponents) const {
        long long maxAlternatingSum = 0;

        for (auto& connectedComponent : parentToConnectedComponents) {
            if (connectedComponent.values.empty()) {
                continue;
            }
