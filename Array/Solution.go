...
func maxAlternatingSum(input []int, swaps [][]int) int64 {
    ...
    var parentToConnectedComponents []ConnectedComponents = createMapParentToConnectedComponents(input, &unionFind)

    return calculateMaxAlternatingSum(parentToConnectedComponents)
}

func createMapParentToConnectedComponents(input []int, unionFind *UnionFind) []ConnectedComponents {
    parentToConnectedComponents := make([]ConnectedComponents, len(input))
    for i := range input {
        parent := unionFind.findParent(i)
        if len(parentToConnectedComponents[parent].values) == 0 {
...
func calculateMaxAlternatingSum(parentToConnectedComponents []ConnectedComponents) int64 {
